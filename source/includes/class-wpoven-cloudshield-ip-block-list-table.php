...
class WPOven_CloudShield_IP_Block_List_Table extends WP_List_Table
{
    private $table_data;
    ...
    function get_columns()
    {
        $columns = array(
            ...
            'ip_status'                => __('IP Status', 'WPOven CloudShield'),
            'blocked_at'                => __('Blocked At', 'WPOven CloudShield'),
    ...
    {
        global $wpdb;
        $table_name = esc_sql($wpdb->prefix . 'cloudshield_blocked_ip_logs');
        ...
        if (isset($_POST['s']) && !empty($_POST['s'])) {
            ...
            $search_columns = array('ip_address', 'status_code', 'ip_status', 'blocked_at', 'timestamp');
    ...
    function column_default($item, $column_name)
    {
        switch ($column_name) {
            ...
            case 'ip_status':
                return $item['ip_status'];
            case 'blocked_at':
                return $item['blocked_at'];
            case 'timestamp':
    ...
    function get_sortable_columns()
    {
        $sortable_columns = array(
            ...
            'ip_status' => array('ip_status', true),
            'blocked_at' => array('blocked_at', true),
    ...
    function views()
    {
        ...
        $table_name = $wpdb->prefix . 'cloudshield_blocked_ip_logs';
        ...
        echo sprintf(
            '<a type="button" style="color:#0073aa;" href="?page=view-cloudshield-ip-block-logs">All&nbsp;(%s)</a>',
            esc_html($row_count)
        );
        echo '</ul>';
    }
